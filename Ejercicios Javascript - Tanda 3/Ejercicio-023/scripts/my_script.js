...
//     let frecuencias=intensidades.map(el=>frecuenciaCardiaca(edad, frecuenciaDescanso, el))
//     return frecuencias
// }

function tablaHTMLMap(intensidades){
    let tabla=`<table><tr><td>Intensidad</td><td>Frecuencia</td></tr><tr><td>---------------</td></tr>`
    tabla+=intensidades.map((el,i)=>`<tr><td>${el}</td><td>${frecuencias[i]}</td></tr>`).join()
    tabla+="</table>"
    return tabla
}

function tablaHTMLReduce(intensidades){
    const frecuencias=intensidades.map(el=>frecuenciaCardiaca(edad, frecuenciaDescanso, el))
    let tabla=""
    tabla+=intensidades.reduce((anterior,actual,i)=>anterior+`<tr><td>${actual}</td><td>${frecuencias[i]}</td></tr>`,
    `<table><tr><td>Intensidad</td><td>Frecuencia</td></tr><tr><td>---------------</td></tr>`)
    tabla+="</table>"
    return tabla
}
...
const intensidades=Array.from({length:intensidadMax+1-intensidadMin},(el,i)=>i+intensidadMin).filter(el=>el%5==0)
const frecuenciaCardiaca=(edad, frecuenciaDescanso, intensidad)=>
    parseInt((((220-edad)-frecuenciaDescanso)*(intensidad/100))+frecuenciaDescanso)
const frecuencias=intensidades.map(el=>frecuenciaCardiaca(edad, frecuenciaDescanso, el))

console.log(`${frecuencias}`)
document.querySelector("body").innerHTML=tablaHTMLMap(intensidades)
document.querySelector("body").innerHTML=tablaHTMLReduce(intensidades)
