...
async function addPostre(postre) {
  ...
  if (resp.ok) {
    ...
    renderCesta(cestas);
    renderDialog(postres, cestas);
  } else {
    alert(`Error al añadir: ${resp.statusText}`);
  }
}

async function updateCantidad(cesta, n) {
  const resp = await ajax({
    url: `${url}/cestas/${cesta.id}`,
    method: "PATCH",
    data: { cantidad: cesta.cantidad + n },
  });
  if (resp.ok) {
...
$cestas.addEventListener("click", async (ev) => {
  ev.preventDefault();
  ...
  if (id) {
    ...
    let resps = await Promise.all(
      cestas.map(
        async (el) =>
          await ajax({ url: `${url}/cestas/${el.id}`, method: "DELETE" })
      )
    );
...
function renderDialog(postres, cestas) {
  let totalPrecio = 0;
...
function renderCesta(cestas) {
  if (cestas.length) {
    if ($cestas.classList.contains("carrito--vacio")) {
      $cestas.classList.remove("carrito--vacio");
    }
    ...
    $cestas.innerHTML = `<h3>Su cesta  <span class="carrito__total"></span></h3>
                       <p class="aviso">Sus articulos añadidos aparecerán aquí</p>`;
    $cestas.classList.add("carrito--vacio");
  }
}
